...
namespace MFST
{
    ...
    Mfst::Mfst(LT::LexTable& lextable, GRB::Greibach pgrebach)
    {
        ...
        for (int k = 0; k < lenta_size; k++)
            lenta[k] = GRB::Rule::Chain::T(lex.table[k].lexem);
        lenta_position = 0;
    ...
    Mfst::RC_STEP Mfst::step(bool show)
    {
        RC_STEP rc = SURPRISE;
        if (lenta_position < lenta_size)
        {
            if (GRB::Rule::Chain::isN(st.top()))
            {
                ...
                if ((nrule = grebach.getRule(st.top(), rule)) >= 0)
                {
                    ...
                    {
                        MFST_TRACE1
                            savestate(show); st.pop(); push_chain(chain); rc = NS_OK;
                        MFST_TRACE2
                    }
                    else
                    {
                        MFST_TRACE4("NS_NORULECHAIN/NS_NORULE")
                            savediagnosis(NS_NORULECHAIN); rc = resetstate(show) ? NS_NORULECHAIN : NS_NORULE;
                    };
                }
                else rc = NS_ERROR;
            }
            ...
            else { MFST_TRACE4("TS_NOK / NS_NORULECHAIN") rc = resetstate(show) ? TS_NOK : NS_NORULECHAIN; };
        }
        else
        {
            rc = LENTA_END;
    ...
    bool Mfst::resetstate(bool show)
    {
        bool rc = false;
        MfstState state;
    ...
    bool Mfst::start(bool show)
    {
        ...
        case LENTA_END:
        {
            MFST_TRACE4("------>LENTA_END")
                cout << endl << "------------------------------------------------------------------------------------------------\n";
            sprintf_s(buf, MFST_DIAGN_MAXSIZE, "%d: всего строк %d, Синтаксис: Синтаксический анализ выполнен без ошибок", 0, lex.table[lex.size - 1].src_str_num);
            cout << setw(4) << left << endl << 0 << "всего строк:  " << lex.table[lex.size - 1].src_str_num << endl << endl << "Синтаксис: Синтаксический анализ выполнен без ошибок\n";
            rc = true;
            break;
        }
    ...
    void Mfst::printrules(bool show)
    {
        if (show == true) {
            cout << endl << "<--------------------Правила грамматики языка AAM-2024-------------------->" << endl;
        }
