...
#define MFST_TRACE_START if(parm.Show==true){cout << setw(4) << left << "Этап" << ":"\
                            << setw(20) << left << " Правило"\
                            << setw(30) << left <<" Входная лента"\
                            << left <<" Стек\n";}

#define MFST_TRACE1         if(show==true){cout << setw(4) << left << ++FST_TRACE_n << ": "\
                            << setw(20) << left << rule.getCRule(rbuf,nrulechain)\
                            << setw(30) << left << getCLenta(lbuf,lenta_position)\
                            << setw(20) << left << getCSt(sbuf)\
                            << endl;}

#define MFST_TRACE2         if(show==true){cout << setw(4) << left << FST_TRACE_n << ": "\
                            << setw(20) << left << " "\
                            << setw(30) << left << getCLenta(lbuf,lenta_position)\
                            << setw(20) << left << getCSt(sbuf)\
                            << endl;}

#define MFST_TRACE3        if(show==true){ cout << setw(4) << left << ++FST_TRACE_n << ": "\
                            << setw(20) << left << " "\
                            << setw(30) << left << getCLenta(lbuf,lenta_position)\
                            << setw(20) << left << getCSt(sbuf)\
                            << endl;}

#define MFST_TRACE4(c)      if(show==true){cout << setw(4) << left << ++FST_TRACE_n << ": "<< setw(20) << left << c << endl;}
#define MFST_TRACE5(c)      if(show==true){cout << setw(4) << left << FST_TRACE_n << ": "<< setw(20) << left << c << endl;}
#define MFST_TRACE6(c,k)    if(show==true){cout << setw(4) << left << FST_TRACE_n << ": "<< setw(20) << left << c << k << endl;}
#define MFST_TRACE7         if(show==true){cout << setw(4) << left << state.lenta_position << ": "\
...
namespace MFST
{
	...
	struct Mfst                     // млакационнный автомат
	{
		...
		bool savestate(bool show);            // сохранить текущее состояние автомата
		bool resetstate(bool show);          // восстановить текущее состояние автомата
		bool push_chain(GRB::Rule::Chain chain); // поместить цепочку правила в стек
		RC_STEP step(bool show);              // выполнить шаг автомата
		bool start(bool show);               // запустить автомат
		bool savediagnosis(RC_STEP pprc_step); // тип возвращаемого значения этапа
		void printrules(bool show);          // выводит промежуточную информацию по правилам
